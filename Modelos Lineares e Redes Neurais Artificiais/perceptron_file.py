...
# Função de ativação degrau (Heaviside function)
def step_function(u):
    ...
    def train(self, training_data, epochs = 10):
        for epoch in range(epochs):
            ...
            for inputs, target in training_data:
                ...
                error = target - prediction         # Cálculo do erro residual
            ...
            # Salva os pesos e bias atuais:
            self.history.append((self.weights[:], self.bias))

            # Exibição com três casas decimais:
            ...
            if global_error == 0:
                ...
                print(f'O treinamento convergiu na época {epoch+1} ✅')
                print(f"Treinamento concluído em {self.converged_epoch} épocas.")
                break
...
# Criação dos dados de entrada de acordo com as tabelas verdades das portas AND, OR e NAND.
def get_training_data(gate_type):
    if gate_type == 'AND':
        ...
        raise ValueError("Porta lógica não suportada.")
